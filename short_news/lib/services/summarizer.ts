...
Règles :
- Répondez avec exactement 3 puces uniquement, chaque ligne commence par "•".
- Chaque puce doit contenir 10 à 18 mots.
- Le total des mots des 3 puces ne doit pas dépasser 60 mots.
...
Rules:
- Return exactly 3 bullet points only, each line must start with "•".
- Each bullet should be 10 to 18 words.
- The total words of all bullet points combined must not exceed 60 words.
...
`;
  ...
  const user =
    lang === "fr"
      ? `
Résumez cet article en 3 puces de 10-18 mots chacune (total max 60 mots), en français, totalement réécrit :

${clean}
`
      : `
Summarize this article in 3 bullet points of 10-18 words each (total max 60 words), in English, fully rewritten:
